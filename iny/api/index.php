...
function handleSaveMemberBeruf(PDO $pdo, string $alliance, string $nameEncoded, array $body): never {
    ...
    $requester = null;
    try {
        $requester = requireR5($pdo, normalizeDiscordId($discordId));
    } catch (\Throwable $e) {
        jsonOut(403, ['error' => 'Kein Zugriff']);
    }
    ...
    if ((int)($requester['current_rank_code'] ?? 0) < 4 && normalizeMemberName((string)($requester['current_name'] ?? '')) !== normalizeMemberName($targetName)) {
        jsonOut(403, ['error' => 'Nur das eigene Profil darf geändert werden']);
    }

    $beruf = strtolower(trim((string)($body['beruf'] ?? '')));
    if ($beruf !== '' && !in_array($beruf, ['ingenieur', 'kriegsherr'], true)) {
        jsonOut(400, ['error' => 'Ungueltiger Beruf']);
    }
    $beruf = $beruf === '' ? null : $beruf;

    $medHilfe = !empty($body['med_hilfe']) ? 1 : 0;
    $winwin = trim((string)($body['winwin'] ?? ''));
    $winwin = $winwin === '' ? null : $winwin;
    ...
    if ($beruf === 'ingenieur' && $winwin !== null) {
        $check = $pdo->prepare("SELECT player_id, current_name, beruf FROM players WHERE alliance = ? AND current_name = ? LIMIT 1");
        $check->execute([$alliance, $winwin]);
        $winwinTarget = $check->fetch();
        if (!$winwinTarget || ($winwinTarget['beruf'] ?? null) !== 'kriegsherr') {
            jsonOut(400, ['error' => 'Win-Win muss ein Kriegsherr sein']);
        }

        $cntStmt = $pdo->prepare("SELECT COUNT(*) AS cnt FROM players WHERE alliance = ? AND beruf = 'ingenieur' AND beruf_winwin = ? AND player_id <> ?");
...
function advanceZugQueue(PDO $pdo, string $alliance, string $rulesetKey, string $memberName, string $eventDate): void {
    ...
    try {
        ...
        // Move member to end (maxPos, which is now free after shift)
...
function handleDeleteZugSchedule(PDO $pdo, string $alliance, string $id): never {
    ...
    // Undo queue advance if entry was still planned and not a substitute
...
try {
    ...
    if (count($segments) >= 2 && $segments[0] === 'zug') {
        ...
        // GET /zug/rulesets
        if ($method === 'GET' && $zugSub === 'rulesets' && $zugId === '') {
            handleGetZugRulesets($pdo, $ALLIANCE);
        }
        // POST /zug/rulesets
        if ($method === 'POST' && $zugSub === 'rulesets' && $zugId === '') {
            requireZugR4($pdo, $ALLIANCE, $body);
            handleSaveZugRuleset($pdo, $ALLIANCE, $body);
        }
        // DELETE /zug/rulesets/{key}
        if ($method === 'DELETE' && $zugSub === 'rulesets' && $zugId !== '') {
            requireZugR4($pdo, $ALLIANCE, $body);
            handleDeleteZugRuleset($pdo, $ALLIANCE, $zugId);
        }
        // GET /zug/queue
        if ($method === 'GET' && $zugSub === 'queue' && $zugId === '') {
            handleGetZugQueue($pdo, $ALLIANCE);
        }
        // POST /zug/queue/sync
        if ($method === 'POST' && $zugSub === 'queue' && $zugId === 'sync') {
            requireZugR4($pdo, $ALLIANCE, $body);
            handleSyncZugQueue($pdo, $ALLIANCE, $body);
        }
        // POST /zug/queue/reorder
        if ($method === 'POST' && $zugSub === 'queue' && $zugId === 'reorder') {
            requireZugR4($pdo, $ALLIANCE, $body);
            handleReorderZugQueue($pdo, $ALLIANCE, $body);
        }
        // GET /zug/schedule
        ...
        if ($method === 'POST' && $zugSub === 'schedule' && $zugId === '') {
            requireZugR4($pdo, $ALLIANCE, $body);
            handleCreateZugSchedule($pdo, $ALLIANCE, $body);
        }
        // PUT /zug/schedule/{id}
        if ($method === 'PUT' && $zugSub === 'schedule' && $zugId !== '' && $zugExtra === '') {
            requireZugR4($pdo, $ALLIANCE, $body);
            handleUpdateZugSchedule($pdo, $ALLIANCE, $zugId, $body);
        }
        // DELETE /zug/schedule/{id}
        if ($method === 'DELETE' && $zugSub === 'schedule' && $zugId !== '' && $zugExtra === '') {
            requireZugR4($pdo, $ALLIANCE, $body);
            handleDeleteZugSchedule($pdo, $ALLIANCE, $zugId);
        }
        // POST /zug/schedule/{id}/noshow
        if ($method === 'POST' && $zugSub === 'schedule' && $zugId !== '' && $zugExtra === 'noshow') {
            requireZugR4($pdo, $ALLIANCE, $body);
            handleNoShowZugSchedule($pdo, $ALLIANCE, $zugId, $body);
        }
        // GET /zug/suggest
        if ($method === 'GET' && $zugSub === 'suggest') {
            handleGetZugSuggest($pdo, $ALLIANCE);
        }
        // GET /zug/my-turns
        if ($method === 'GET' && $zugSub === 'my-turns') {
            handleGetZugMyTurns($pdo, $ALLIANCE);
        }
